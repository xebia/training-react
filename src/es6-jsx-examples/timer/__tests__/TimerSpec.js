...
describe('Component: Timer', () => {
  ...
  it('should render the current time based on the state', () => {
    ...
    expect(wrapper.find('p').text()).toBe(`The time is ${new Date(10000)}`);
  });

  it('should update state.time every 500ms', () => {
    expect(wrapper.state()).toEqual({ time: new Date(0) });
    sandbox.clock.tick(499);
    expect(wrapper.state()).toEqual({ time: new Date(0) });
    sandbox.clock.tick(1);
    expect(wrapper.state()).toEqual({ time: new Date(500) });
    sandbox.clock.tick(500);
    expect(wrapper.state()).toEqual({ time: new Date(1000) });
  });
